...
public class UserController {
	...
	public ResponseEntity<?> getAllDeptList(){
		...
		List<DepartmentList> l1 = userDao.getAllDepartment();
	...
	public ResponseEntity<?> allDoctorDepartmentWise(@PathVariable(name="id") int id){		
		...
		List<Doctors> l1Doc = userDao.getDoctorDeptWise(id);
	...
	public ResponseEntity<?> addAppointmentNew(@RequestBody AppointmentDTO appDto){
		...
		try {
			return new ResponseEntity<PatientDto>(userDao.registerAppointmentNew(appDto), HttpStatus.OK);
	...
	public ResponseEntity<?> addAppointmentOld(@RequestBody AppointmentDTO appDto) {
		...
		System.out.println("addAppointmentOld() in /admin/app------------");
		try {
			return new ResponseEntity<PatientDto>(userDao.registerAppointmentOld(appDto), HttpStatus.OK);
	...
	public ResponseEntity<?> getAllPatient() {
		...
		List<PatientDto> l2 =  userDao.getAllPatient();
		try {
	...
	public ResponseEntity<?> onLogin(@RequestBody User user){
		try {
			if(user.getRole().equals("Admin")) {
				return new ResponseEntity<User>(userDao.validateAdmin(user), HttpStatus.OK);
			} else if(user.getRole().equals("Patient")) {
				return new ResponseEntity<User>(userDao.validatePatient(user), HttpStatus.OK);
			} else if(user.getRole().equals("Doctor")) {
				return new ResponseEntity<User>(userDao.validateDoctor(user), HttpStatus.OK);
			}	else {
